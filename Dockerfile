...
# docker build -t spreent .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name spreent spreent
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential default-libmysqlclient-dev git pkg-config redis && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
